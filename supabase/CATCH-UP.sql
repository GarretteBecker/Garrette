...
--
-- 'ops' is the office: members, scheduling, requests, reports, trades.
-- Not pricing, not user accounts. Everything below compares roles as
-- ::text rather than as enum literals, which is what lets the new value
-- be added and used in this single transaction.
--
-- ⚠ Sign-up becomes INVITE ONLY from here. Anyone without an unexpired
-- row in public.invites is refused at the database, whatever the Supabase
-- dashboard says. Your existing accounts are untouched — the check only
-- runs when an account is created.
-- ---------------------------------------------------------------------
...
drop policy if exists invites_select_staff on public.invites;
...
--
-- The Miller Home from the seed is a REAL row in your database. Without
-- this it is counted as a member, added to your monthly recurring revenue
-- and listed for renewal. Flagged rather than deleted, because a demo on
-- a phone in somebody's kitchen needs data real enough to click through.
-- ---------------------------------------------------------------------
...
select
  case
    ...
    when not exists (
      ...
      then 'The team console did not apply — send this result to Claude.'
    when not exists (
      select 1 from information_schema.columns
       where table_schema = 'public' and table_name = 'properties' and column_name = 'is_demo'
    )
      then 'The demo flag did not apply — send this result to Claude.'
    else 'Up to date. Everything through the team console is in. Sign-up is now invite only.'
  end as result,
